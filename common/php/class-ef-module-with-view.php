...
class EF_Module_With_View extends EF_Module {
	...
	public function is_module_settings_view( $slug = false ) {
		...
		if ( empty( $settings_view_slugs ) || ! in_array( $_GET['page'], $settings_view_slugs, true ) ) {
			return false;
		}
	...
	function get_current_post_type() {
		...
		} elseif ( in_array( $pagenow, array( 'edit.php', 'post-new.php' ), true ) && empty( $_REQUEST['post_type'] ) ) {
			$post_type = 'post';
		} else {
			$post_type = null;
		}
	...
	public function is_whitelisted_functional_view( $module_name = null ) {
		_deprecated_function( __FUNCTION__, '0.9.3', 'is_active_view' );
		return true;
	}
